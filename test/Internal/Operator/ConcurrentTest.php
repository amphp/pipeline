...
class ConcurrentTest extends AsyncTestCase
{
    public function testNoValuesEmitted(): void
    {
        ...
        $pipeline = $source->pipe()->pipe(
            Pipeline\concurrent(3, Pipeline\map($this->createCallback(0)))
        );
    ...
    public function testConcurrency(): void
    {
        ...
        $pipeline = $source->pipe(
            Pipeline\concurrent(3, Pipeline\tap(fn (int $value) => delay(\random_int(0, 10) / 1000)))
        );
    ...
    public function testPipelineFails(): void
    {
        ...
        $pipeline = $source->pipe()->pipe(
            Pipeline\concurrent(3, Pipeline\tap($this->createCallback(1)))
        );
